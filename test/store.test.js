...
test('migrates a v1 database while preserving observations and adding estimation tables', (t) => {
  ...
  try {
    assert.equal(reopenedDb.prepare('PRAGMA user_version').get().user_version, 10);
...
function makeV2Fixture(dbPath, invalidRegistry = false) {
  ...
  store.close();
  const db = new DatabaseSync(dbPath);
  db.exec('DROP TABLE estimation_inputs; DROP TABLE estimation_runtime; DROP TABLE daily_usage; DROP TABLE monthly_usage; DROP TABLE history_fetch_state; DROP TABLE shared_estimation_events; DROP TABLE shared_estimation_state; DROP TABLE device_contracts; DROP TABLE contracts; PRAGMA user_version = 2;');
