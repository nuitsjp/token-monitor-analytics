...
| T-032 | 隣接差分と推定エンジン | T-031、SP-04 | 隣接差分、等値制約、倍率、列正規化、階数、非負最小二乗、推定済み・モデル不適合を含む六状態分類。階数判定、解法、有限かつ正の一意解判定を計算論理版に含め、AC-P1-13 から AC-P1-19 を要件値のまま固定試験にする |
...
| ID | タスク | 依存 | 成果物・受入れ |
| --- | --- | --- | --- |
| T-060 | 利用実績観測の正規化と保存 | T-012、T-051 | 対応 API 契約の `devices[].periods.allTime` から、利用額ソース単位の累積トークン数、API 換算利用額、モデル内訳を元観測として保存する。加えて `periodWindows.today` / `month` と `periods.today` / `month` から取得元期間観測を保存する。観測時刻、取得元タイムゾーン・期間キー・期間終了時刻、原 JSON と JSON パス、正規化世代を保持し、同じ Hub・端末・期間種別・期間キー・端末利用更新時刻の同値だけを重複排除する。別 Hub の値を内容一致で除外せず、未知フィールドを推測して解釈しない |
| T-061 | 利用実績集計と集計根拠 | T-023、T-060 | 当日・当月は取得元期間観測を正本とし、表示タイムゾーンと期間キーが一致する端末ごとの最新有効値だけを合算する。任意期間は同一利用額ソースの隣接する有効累積観測の観測間増分から集計し、開始と終了が同じ対象バケットに含まれない差分は暦バケットへ配分しない。Hub、収集端末、Hub 端末レコード、正式サービス、生利用額サービス識別子、モデルを分類軸にし、単一アカウント帰属と共有利用実績を区別して共有分を按分しない。各集計行から使用した元観測、原 JSON、関連付けへ遡れる集計根拠を返す |
...
| ID | タスク | 依存 | 成果物・受入れ |
| --- | --- | --- | --- |
| T-063 | M01・M02・M03・M06・M08 の Phase 2 表示 | T-061、T-062 | M02 の期間・粒度・分類フィルター、サマリー、推移、内訳、共有利用実績、未配分の観測間増分、集計根拠への導線を実装する。M01 と T01 の当日・当月は取得元期間観測を表示し、採用した観測時刻を画面生成時刻と混同しない。M03 に月間換算・標準価格・価値倍率・履歴、M06 に標準価格編集、M08 に集計根拠を追加する。M02 の期間指定箇所だけに表示タイムゾーン名を常設する |
