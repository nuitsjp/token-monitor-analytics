...
| ID | 確認内容 | 完了条件 |
| --- | --- | --- |
| SP-01 | Hub API 契約ポリシー | `hubBuild` の schema、runtime、最低 core revision と `usageUpdatedAt` の更新・据置きを実在 Hub で確認し、未知フィールドを含む有効応答、無効応答、不正応答のフィクスチャを固定する。接続・応答タイムアウトと応答本文の最大バイト数も決定する。`usageUpdatedAt` がない対応 Hub は収集専用契約として扱い、利用上限推定だけを非対応とする |
