...
2026-08-25 に、最初のリリースをクラウド機能を含まない Windows ローカル版とすることを合意した。対象は本書の T-001 から T-051、および要件定義の `P1-*`、`QL-*`、`AC-P1-*` とする。クラウド同期、クラウド保存、サーバー実装などの `P2-*` は対象外とする。
...
完了条件は T-001 から T-051 の成果物を実装し、`AC-P1-01` から `AC-P1-26`、画面受入キー、デザインシステムの共通規則を `wails3 task acceptance` で確認できることとする。`P2-*` は後続リリースへ送り、本リリースのローカルファースト境界にクラウド依存を持ち込まない。
