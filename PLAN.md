...
| ID | 確認内容 | 完了条件 |
| --- | --- | --- |
| SP-01 | Hub API 契約と許可リスト | `hubBuild` の分類と `usageUpdatedAt` の更新・据置きを実在 Hub で確認し、未知フィールドを含む有効応答、無効応答、不正応答のフィクスチャを固定する。接続・応答タイムアウトと応答本文の最大バイト数も決定する。`usageUpdatedAt` がない許可済み Hub は収集専用契約として扱い、利用上限推定だけを非対応とする |
...
| リスク | なぜ起きるか | ゲート |
| --- | --- | --- |
| `usageUpdatedAt` を提供する対応 Hub が存在しない | 推定観測時刻を取得時刻で代用すると要件の根拠が崩れる | 許可済み Hub の収集・原 JSON・利用枠観測は継続し、利用額を使う推定だけを非対応として明示する |
