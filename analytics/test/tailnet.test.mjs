...
test('tailnet mode requires a named tailnet, explicit CGN address and restricted viewer modes',t=>{
 ...
 f.raw.demo=false;f.raw.viewerAuth={mode:'tailscale'};f.write();assert.ok(loadConfig(f.file));
 const viewer=f.raw.tailnetViewer;delete f.raw.tailnetViewer;f.write();assert.throws(()=>loadConfig(f.file));f.raw.tailnetViewer=viewer;
...
test('two actual listeners share SQLite and SSE; tailnet ingest is always blocked',{skip:address?false:'Requires an assigned Tailscale interface',timeout:15000},async t=>{
  ...
  const stream=await send('/api/live',{signal:ctrl.signal});assert.equal(stream.status,200);const reader=stream.body.getReader();
  ...
  assert.equal((await send('/api/state',{headers:{Authorization:basic}})).status,200);
  ctrl.abort();
  await app.close();
});
