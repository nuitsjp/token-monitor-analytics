...
const when=t=>t?new Date(t).toLocaleString('ja-JP',{timeZone:'Asia/Tokyo',hour12:false}):'—';
...
async function get(path){const r=await fetch(path,{cache:'no-store'});if(r.status===401)throw new Error('Cloudflare Accessへのログインが必要です。再読み込みして認証してください。');if(!r.ok)throw new Error(`APIの読み込みに失敗しました (${r.status})。保存済みの表示は更新されていません。`);return r.json();}
let data=null,view='dashboard',ws=null,retry=1000,retryTimer=null,stopped=false,lastPong=0,loading=false,dirty=false;
...
function draw(){if(!data)return;
 ...
 $('observed').textContent=h?`最終観測 ${when(h.observedAt)} JST${age>=5?` · ${age}分前の観測値`:''}`:'まだ観測データがありません。Collectorのログと接続設定を確認してください。';
...
function chart(rows){const valid=rows.filter(r=>r.monthly_capacity_usd!==null).reverse();if(!valid.length){$('chart').append(el('p','有効な推定が得られると、日次の月換算推移を表示します。','muted'));return;}
 ...
 for(const r of valid){const day=Date.parse(r.day),x=70+(day-start)/span*680,y=150-r.monthly_capacity_usd/max*125;if(lastDay&&day-lastDay>86400000)flush();segment.push(`${x},${y}`);svg.append(make('circle',{cx:x,cy:y,r:3}));lastDay=day;}flush();
 for(const [text,x,y]of [[money(max),0,24],['$0',0,154],[valid[0].day,70,177],[valid.at(-1).day,680,177]]){const t=make('text',{x,y});t.textContent=text;svg.append(t);}$('chart').append(svg);}
function connection(s,on=false){$('live').textContent=s;$('dot').classList.toggle('on',on);}
function connect(){if(stopped||ws?.readyState===WebSocket.OPEN||ws?.readyState===WebSocket.CONNECTING)return;clearTimeout(retryTimer);const u=new URL('/api/live',location.href);u.protocol=location.protocol==='https:'?'wss:':'ws:';ws=new WebSocket(u);const socket=ws;
 socket.onopen=()=>{retry=1000;lastPong=Date.now();connection('ライブ接続中',true);};
 socket.onmessage=e=>{if(e.data==='pong'){lastPong=Date.now();return;}try{const m=JSON.parse(e.data);if(m.type==='ready'||m.type==='updated')refresh();}catch{}};
 socket.onclose=e=>{if(ws!==socket)return;ws=null;connection('ライブ切断');if(stopped)return;if(e.code===1008){notice('閲覧セッションが終了しました。画面を再読み込みしてください。');return;}retryTimer=setTimeout(connect,retry+Math.random()*500);retry=Math.min(30000,retry*2);};
 socket.onerror=()=>connection('ライブ再接続待ち');}
setInterval(()=>{if(ws?.readyState===WebSocket.OPEN){if(Date.now()-lastPong>75000){ws.close();return;}ws.send('ping');}},30000);
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden){refresh();connect();}});window.addEventListener('pagehide',()=>{stopped=true;clearTimeout(retryTimer);ws?.close();});
refresh();connect();
