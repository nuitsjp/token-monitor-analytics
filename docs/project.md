...
UC-1-Mは2 Hub・設定ファイル対応の実装、完成系承認、制御可能なHubでの段階6 E2Eまで完了しました。実Hubとの相互接続とLinux/CIは未検証です。基盤の検証と製品の受け入れ検証は区別します。
