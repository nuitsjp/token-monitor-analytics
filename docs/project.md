...
UC-1は保存確定後に閲覧側へ最新状態を通知する改訂の完成系承認を得て、段階6の検証中です。UC-2の保存済みHub・デバイスを一度読み取るダッシュボードは完了しています。画面の自動更新はUC-1の改訂完了後に扱い、他のパーツは固定サンプルから段階的に実装します。旧製品の機能を暗黙に復元しません。
...
UC-1の保存後通知は、起動中に `curl.exe -N http://127.0.0.1:3000/api/usage/stream` で確認します（PORT変更時は読み替えます）。接続時とHubの保存成功後に `event: update` と閲覧用全体状態のJSONが届きます。Ctrl+Cで閲覧接続だけを閉じ、再実行すると最新保存値から再開します。UC-2の画面への自動反映は未実装です。UI確認不要の系列のため、通知の監査には画面やPlaywright CLIを使いません。

<a id="verification"></a>
## 6. 検証結果

UC-1-Mの2 Hub・設定ファイル対応は完成系承認、段階6 E2E、実際の2 Hubとの相互接続まで完了しました。保存後通知の改訂は段階5で承認され、段階6の検証中です。実Hubでの保存後通知は確認済みで、新しい通知のE2E・長時間低速接続の実測とLinux/CIは未検証です。基盤の検証と製品の受け入れ検証は区別します。
...
`verify` は設定の基盤テスト、Lint、文書、型、本番ビルド、UC-1・UC-2の製品E2Eを実行します。CIも同じコマンドを使用しますが、この変更のCI実行結果は未検証です。

UC-2-MのE2Eは登録Hubの絞り込み、未受信と再読込、期間別の整数表示・料金・構成比、秘密情報非公開、閲覧時のDB不変、自動更新なし、API失敗、受信済みのゼロ値を検証します。実Hub検証は既存DBと別の検証用DBで実施しました。ツール・モデル・利用枠・トレンド等は固定サンプルで、受け入れ範囲はHub・デバイスです。
