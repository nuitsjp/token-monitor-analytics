...
| --- | --- | --- | --- | --- | --- | --- |
| UC-1-M | 4 | Windows / Node.js 24.19.0 / 外部HubをローカルSSEに置換・本番entry・一時SQLite | 2026-09-20 | Node REPLから本番entryを起動し、通知送信と別の読み取り専用DB接続で手動確認 | 合格 | `ec9ca61` |
| UC-1-M | 4 | 基盤検証 | 2026-09-20 | `npm run verify` | 合格 | `ec9ca61` |

手動確認では、未受信時のHub登録、snapshot保存、stats全体置換と端末削除、重複時の非加算、freshnessで利用量・上限維持、heartbeatで無更新を確認しました。UTF-8・CRLFの分割と複数data行も受信できました。不正通知、初回snapshot前の差分、DB更新失敗、HTTP 401、通信断では保存状態を維持して受信だけ停止し、Web稼働を確認しました。再起動後の保存状態、接続中の通常終了、認証情報が保存内容・ログに出ないこと、移行成功と移行失敗時のロールバックも確認しました。UI確認不要の系列なので、Playwrightによる画面操作は省略しています。製品テストコードは段階5の承認前のため作成・変更していません。
