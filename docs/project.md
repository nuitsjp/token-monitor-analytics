...
UC-1は保存確定後に閲覧側へ最新状態を通知する改訂を実装します。UC-2の保存済みHub・デバイスを一度読み取るダッシュボードは完了しています。画面の自動更新はUC-1の改訂完了後に扱い、他のパーツは固定サンプルから段階的に実装します。旧製品の機能を暗黙に復元しません。
...
UC-1-Mの2 Hub・設定ファイル対応は完成系承認、段階6 E2E、実際の2 Hubとの相互接続まで完了しました。保存後通知の改訂は段階1承認済みで、改訂後の検証は未実施です。Linux/CIの実行結果は未検証です。基盤の検証と製品の受け入れ検証は区別します。
