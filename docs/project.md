...
利用者がHubの最新情報をローカルで利用できるよう、UC-1の受信・保存を完了しました。次にUC-2で、保存済みのHub・デバイスをA案のダッシュボードへ表示します。他のパーツは固定サンプルから段階的に実装します。旧製品の機能を暗黙に復元しません。
...
UC-2-Mの段階4では、A案の固定パーツと、SQLiteから読み取るHub・デバイス欄を本番ビルドで確認しました。API応答が現在の接続設定にあるHubだけを返し、接続先URLと認証トークンを含まないことも確認しました。UC-2のE2Eは段階5の完成系承認後に追加します。
