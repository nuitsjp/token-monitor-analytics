...
UC-1のE2Eは、制御可能な2つのSSE Hubから本番Nodeプロセスを通り、別の読み取り専用接続でSQLiteの保存結果までを確認します。UC-2のE2Eは同じ構成のHubから本番Nodeプロセスとブラウザーを通り、画面の表示値までを確認します。テストごとにHub、プロセス、ポート、設定ファイル、DBを分離し、`verify` とCIへ組み込んでいます。設定の基盤テストはテンプレートから継承しています。
