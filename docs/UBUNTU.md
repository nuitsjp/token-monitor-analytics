...
この手順はUbuntu/systemd実機での受入試験を代替しません。停止・起動、OS再起動、24時間連続、実Hubの料金・利用率対応、バックアップからの復元は、利用者のUbuntuで実行して結果を記録してください。この手動配置手順の`scp`とシステムサービス方式は実地検証していません。miseによるユーザーサービス方式の正式発行結果は[VERIFICATION](VERIFICATION.md)を参照してください。
