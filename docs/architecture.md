...
UC-1の完了済み設計と、UC-2の合意済み設計を記録します。UC-1の2 Hub・設定ファイル対応は完成系承認、E2E、実Hub検証まで完了しています。実装済みの基盤は [React構成](architecture-react.md)、テンプレート採用範囲は [文書方針](document-policy.md#adoption) を参照します。
...
- UI確認用モックは不要です。検証時の置き換え境界は外部Hubとし、制御可能なSSEサーバーから本番の受信・保存処理を通し、別のDB接続で保存結果を確認します。実Hubとの相互接続も実施済みです。詳細は [検証結果](project.md#verification) を参照します。
...
| 役割         | 責務                                                                                              | 実装パス    |
| ------------ | ------------------------------------------------------------------------------------------------- | ----------- |
| 利用状況読取 | 登録Hubと最新状態を読み、画面へ必要な値だけを返す                                                 | 段階4で確定 |
| 利用状況画面 | 読み取りAPIを一度呼び出し、受信済みHubの合計、Hubごとのカード、または画面全体の取得失敗を表示する | 段階4で確定 |
...
sequenceDiagram
  ...
  D-->>A: Hubごとの最新状態
  A-->>V: 表示に必要な値だけを返す
  V-->>U: 全Hub合計とHubごとのカードを表示
```

読み取りAPIの応答完了を結果確定点とし、状態は更新しません。画面は応答内の受信済みHubだけを合算し、未受信Hubを合計へ加えません。DB読み取りまたはAPI呼び出しに失敗した場合は、画面全体に取得失敗を表示します。モックではtRPCクエリの結果を合成点として本番の応答型を共有し、受信済みHubと未受信Hubを含む固定データへ差し替えます。段階4で固定データを削除し、SQLiteの実処理へ接続します。
...
| ADR-5 | UC-2は登録Hub数を固定せず、保存済みの最新利用状況を一度読み取ってカード表示する   | 2026-09-20、UC-2-M全文案への応答「OKです。それで進めてください」                       | UC-2-M。自動更新、詳細内訳、履歴表示は含めない |
