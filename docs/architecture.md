...
| <a id="s9"></a>[S9](architecture/runtime-view.md#s9) | 手動更新と障害復旧 | 利用者が終了・`git pull`・起動を行い、起動時に永続化層が版を確認して移行を 1 トランザクションで行う。失敗時は移行前の版とデータを保ち、原因と復旧情報をログに出して起動を中止する。系列は [P-2](#p-2)、復旧手順は [U8](../PLAN.md#u8) |
...
| 役割 | 責務 | 実装パス |
| --- | --- | --- |
| 起動制御 | 起動・終了の受付、アプリの版の取得、起動結果と失敗原因・復旧情報のログ出力 | src/main.js |
| 実行環境 | 排他起動、設定読込、起動失敗時の解放 | src/runtime.js |
| 永続化 | スキーマ版の確認、移行の単一トランザクション実行、失敗時の版と段階の報告 | src/store.js |
| 取り込み制御 | アプリの版とスキーマ版を状態に含めて配信 | src/app.js |
| 画面 | フッターに版を表示 | public/app.js |
...
sequenceDiagram
  ...
  利用者->>起動制御: Ctrl+C で終了
  利用者->>利用者: git pull
  利用者->>起動制御: node src/main.js real
  起動制御->>起動制御: アプリの版を取得
  起動制御->>実行環境: 起動
  実行環境->>永続化: DB を開く
  永続化->>永続化: 版確認・移行（1 トランザクション、結果確定点）
  永続化-->>実行環境: 移行前後の版
  実行環境-->>起動制御: 起動完了
  起動制御->>起動制御: 版と移行結果をログへ記録
  取り込み制御-->>画面: 版を含む状態を SSE で配信
```
