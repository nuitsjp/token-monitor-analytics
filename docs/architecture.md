...
- **重複排除**: 同一内容の再受信によって新たな増分や推定値を計上せず、同一日次・月次実績の再取得時も上書き更新する。受信時刻や通信経路の違いを新規実績の根拠としない。
...
| U6 | 受信処理とタイムゾーン | 独立受信・直列保存、端末現地日付の維持、収集設定の永続化。 | タイムゾーン欠落時の当日判定、収集設定を含む SQLite スキーマ、トランザクション境界。 |

## 6. 受信データの処理と保存

SSE（現在値観測・推定）と GET（履歴収集）は独立して並行受信し、受信後は **共通の単一キュー** に投入して 1 件ずつ直列に処理・保存する。SQLite への保存成功後にブラウザへ更新通知を配信する。

※ 1 件の処理単位は、1 回の GET 応答または 1 回の SSE データ通知（複数端末分を含む場合も 1 処理単位）。
...
| 通信失敗と復帰 | GET 失敗時は 1 時間間隔で再試行され、成功後は不要な再試行が行われないこと。 |
...
利用者が特定の Hub からの収集を停止・再開する際の動作仕様（上流 Hub の稼働状態とは独立して管理）。

| 区分 | 状態と動作 |
| --- | --- |
| 未接続 | 収集停止操作を行っていない状態で Hub に接続できない場合。原因を問わず自動再接続を継続。 |
| 収集停止 | 利用者が停止操作を行った場合。再開操作まで該当 Hub の全通信（SSE、自動再接続、定期 GET）を停止。 |

※ 収集停止中も、保存済みデータの閲覧や他 Hub の収集は通常どおり継続する。

#### 停止操作時のデータ処理境界
停止要求の受付時点をデータ取り込みの境界とする（1 回の応答・通知単位で判定）。

- **受信完了済みのデータ**: キューで待機中・処理中のものを含め、通常どおり処理・保存する。
- **受信途中の通信**: 完了を待たずに中止し、遅れて届いた応答は破棄する。
- **処理完了後の表示**: 受信済みデータの処理終了後に「収集停止」と表示。保存失敗時は書き込みを停止し「収集停止」「保存失敗」を併記する。

#### 収集設定の保存と再起動時の復元
利用者が指定した Hub ごとの収集設定（有効・停止）は SQLite に保存し、再起動後も引き継ぐ。起動時は通信開始前に設定を読み込む。

| 保存された収集設定 | 起動後の動作 |
| --- | --- |
| 有効 | 自動で SSE 接続を開始。接続できない場合は「未接続」と表示し自動再接続を継続する。履歴 GET は第 6.1 節に従う。 |
| 停止 | 「収集停止」を維持し、利用者の明示的な再開操作まで全通信を停止したままとする。 |

※ 実際の接続状態は起動後の通信結果から判定し、再起動前の接続状態を現在の状態として表示しない。
