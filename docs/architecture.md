...
読み取りAPIの応答完了を結果確定点とし、状態は更新しません。現在の接続設定にあるHubだけを対象に、受信済みHubのデバイスと期間別トークン数・推定コストを返します。期間切替と最大Hubを基準とする棒の比率は、取得済みの応答から画面で計算します。DB読み取りまたはAPI呼び出しに失敗した場合は、Hub・デバイス欄に取得失敗を表示します。モックではtRPCクエリの結果を合成点として本番の応答型を共有します。A案の他のパーツは将来の実装位置を示す固定サンプルであり、APIの応答には含めません。
