...
3. リセット後は新しい対象期間の最初の有効な観測を起点にする。リセットをまたぐ場合や途中で対応関係が不明になった場合は、その区間を計算対象にしない。
...
U2・U3 の初期方針は合意済みである。手順 1 には U2 の具体的な識別設計と U4・U5 の確認・影響整理が残る。U3 の測定時刻のずれを許容する方針について、連続観測による許容秒数の決定を初期設計の完了条件にはしない。
...
今回の調査で上流のデータ形式と補完の限界を特定し、利用者との検討により U2・U3 の初期方針を確定した。手順 1 全体は、残る識別設計と U4・U5 の確認・影響整理を終えていないため未完了である。
