...
| 利用状況画面 | 読み取りAPIを一度呼び出し、Hubごとのカードまたは画面全体の取得失敗を表示する | 段階4で確定 |
...
読み取りAPIの応答完了を結果確定点とし、状態は更新しません。DB読み取りまたはAPI呼び出しに失敗した場合は、画面全体に取得失敗を表示します。モックではtRPCクエリの結果を合成点として本番の応答型を共有し、受信済みHubと未受信Hubを含む固定データへ差し替えます。段階4で固定データを削除し、SQLiteの実処理へ接続します。
