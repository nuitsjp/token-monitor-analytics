...
- **現在の実装範囲**: 起動・設定、現在値の受信・検証・保存・表示、再接続、全 provider 共通の契約・利用枠推定、Cursorのアカウント全体実績の重複除去、欠測端末を除外する部分推定、Grokの既合意メールキー、共通・旧方式の履歴API/UIを実装しています。provider と同IDの `clientCosts`・`clientHealth` を共通入力とし、枠ごとの消費率を独立した参考値として扱います。`npm test` 148件、schema 6の実DB・Mock移行と保全照合、実データ再起動、headless Edgeの390px・1440px表示を確認済みです。全サービスの実契約を個別に検証することは要件に含めず、正規化した共通入力のテストで担保します。残りの機能と検証範囲は [PLAN.md](../PLAN.md) で追跡します。
...
| 共通比較基準・推定結果 | 採用した論理利用額ソースの起点・最新観測、除外ソースと理由、算出時点、対象区間、推定不可理由、算出根拠を保持。`percentageLimit` と論理ソース集合の変更を境界とする | `shared_estimation_state` にmethodVersion 2として実装。schema 6の実DB移行・UIを検証済み |
...
schema 3・4・5 の移行処理と検証は過去の事実として扱います。schema 6の `migrateUsagePolicy` は、全サービスの旧checkpointの比較基準を破棄し、保存済み現在値から `methodVersion: 2` の状態をseedします。旧viewは `active: false`・`reason: method_changed` として `lastResult` を保持し、現在の比較へ流用しません。観測・現在値・旧方式と共通方式の推定events・契約関連を含む8表は変更せず、過去の結果を遡及再計算しません。非互換・破損時の自動初期化は行いません。
...
schema 6 でも次の共通表の構造と責務を維持します（schema 3・4・5で追加・再構成した経緯は歴史的な移行記録です）。
...
- **推定状態の再開**: 起動時に既存の推定系列へ restart の欠測境界を記録します。保存済みの過去結果を保持し、次の正常な通知で欠測端末を除外した論理利用額ソースと契約から新しい起点を作ります。古い観測を読み直して遡及計算しません。
...
**過去の設計判断：Hub横断の共通契約とschema 3（2026-09-13）:** 実データで同じCodex Pro5x契約が複数Hubに現れ、Hub別方式のままでは契約枠の消費率を重複して扱います。画面上のカードを統合するだけでは、推定に必要な全Hubの利用額と契約枠の対応を修正できません。そのため、Hub・端末・ツールと契約の関係をn:nで永続化し、全関係ソースから共通推定を算出しながら、旧方式の履歴を保持するschema 3移行を行いました。その後、schema 5で全 provider の契約をHub横断へ統合し、現在はschema 6で利用実績の収集範囲を考慮する方式へ更新しています。
