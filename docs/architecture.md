...
適用系列: [UC-2-M・UC-2-X1・UC-2-X2・UC-2-X4](usecases/UC-2.md)。現在の接続設定に登録されているHubだけを表示対象とし、表示処理ではHub数を固定しません。

| 役割         | 責務                                                                                              | 実装パス    |
| ------------ | ------------------------------------------------------------------------------------------------- | ----------- |
| 利用状況読取 | 登録Hubと最新状態を読み、Hub・デバイス・利用枠の表示に必要な値だけを返す                            | backend/http/router.ts、backend/db/hub-state.ts |
...
合成点は `frontend/src/features/usage-overview.ts` の取得・購読境界です。本番の `UsageOverview` を共有し、初回取得はtRPC、以後は同一originの `/api/usage/stream` を使います。仕様合意用の固定データとモック切替は削除済みです。トレンド・ツール・モデル・アクティビティは固定サンプルです。DB変更はありません。
