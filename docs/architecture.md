...
1. Hub ごとの収集ロックを取得し、schema、runtime、最低 core revision の契約ポリシーに従って **/api/health** を検証する。
...
- URL は絶対 URL に限定し、`userinfo`、`query`、`fragment` を拒否する。host が `localhost`、ループバック IP、またはプライベート IP のリテラルである場合は HTTP を許可し、それ以外は証明書検証を有効にした HTTPS だけを許可する。HTTP は利用者が管理する信頼済み LAN に限定する。
...
- 各収集の前に /api/health を取得し、schema、runtime、最低 core revision の契約ポリシーへ一致した場合だけ、認証済み /api/stats を一度呼ぶ。build ID は互換性判定に使わず、実際に応答したビルドの追跡情報として保存する。
