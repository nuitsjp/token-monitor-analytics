...
| 端末 | Hub の登録単位と `deviceId` を組み合わせて分離する。 | 再インストール、ID 再利用、収集範囲変更時の実績の継続性。 |
...
| U5 | 複製ログを複数端末から報告する構成をサポート対象外とする方針を合意済み。再インストール、端末 ID の再利用、収集範囲変更時の実績の継続性は未確定。 | 複製ログの扱いは第 3.2 節。残る継続性の扱いは、上流の端末 ID・収集設定と累積実績の挙動を確認して定義する。 | 端末間の複製ログの自動検出・除外は実装しない。SSE 再受信・GET 補完の二重計上防止は維持する。端末再設定や収集範囲変更の前後で採用する比較基準は残る設計事項。 |
| U6 | GET と SSE の競合、保存・通知の順序、永続化する比較基準。 | 本調査を入力に PLAN 手順 3 のシナリオを設計する。 | SQLite の一意性制約、排他方式、トランザクション境界。手順 1 では確定しない。 |

U2・U3 の初期方針、U4 の期間判定方針、U5 の複製ログの扱いは合意済みである。手順 1 には U2 の具体的な識別設計、U4 の対象サービス・プランと枠の識別詳細、U5 の端末再設定・収集範囲変更時の実績の継続性の整理が残る。U3 の測定時刻のずれを許容する方針について、連続観測による許容秒数の決定を初期設計の完了条件にはしない。
...
今回の調査で上流のデータ形式と補完の限界を特定し、利用者との検討により U2・U3 の初期方針、U4 の期間判定方針、U5 の複製ログの扱いを確定した。手順 1 全体は、残る対象・識別の設計と U5 の実績の継続性の整理を終えていないため未完了である。
