...
UC-1の保存後通知を含む承認済み設計と、UC-2の一度読み取る閲覧設計を記録します。実装済みの基盤は [React構成](architecture-react.md)、テンプレート採用範囲は [文書方針](document-policy.md#adoption) を参照します。
...
受信・保存・配信は単一Node.jsプロセスで行い、SQLiteを保存済み状態の正本とします。UC-1は閲覧側への配信までを担当します。後続のUC-2ではブラウザーごとに共通接続を1本持ち、受信した `UsageOverview` を既存のTanStack Queryキャッシュへ反映する方針です。複数コンポーネントは同じキャッシュを購読し、期間選択等の操作状態は分離します。ブラウザー側の接続は未実装です。
...
| 最新状態の通知処理 | 登録Hubの保存済み閲覧用データを生成し、接続時と保存確定後にSSEで配信 | 段階4で確定 |
...
- UI確認用モックは不要です。検証時の置き換え境界は外部Hubとし、制御可能なSSEサーバーから本番の受信・保存・配信処理を通し、別のDB接続の保存値と閲覧側の受信内容を照合します。保存後通知の改訂は未検証です。詳細は [検証結果](project.md#verification) を参照します。
...
適用系列: [UC-2-M](usecases/UC-2.md)。現在の接続設定に登録されているHubだけを表示対象とし、表示処理ではHub数を固定しません。

| 役割         | 責務                                                                                              | 実装パス    |
| ------------ | ------------------------------------------------------------------------------------------------- | ----------- |
| 利用状況読取 | 登録Hubと最新状態を読み、Hub・デバイスの表示に必要な値だけを返す                                  | backend/http/router.ts、backend/db/hub-state.ts |
| 利用状況画面 | 読み取りAPIを一度呼び出し、Hub・デバイス欄または同欄の取得失敗を表示する                           | frontend/src/routes/index.tsx |
...
読み取りAPIの応答完了を結果確定点とし、状態は更新しません。現在の接続設定にあるHubだけを対象に、受信済みHubのデバイスと期間別トークン数・推定コストを返します。期間切替とHub全体のトークン数・推定コストそれぞれの構成比は、取得済みの応答から画面で計算します。DB読み取りまたはAPI呼び出しに失敗した場合は、Hub・デバイス欄に取得失敗を表示します。モックではtRPCクエリの結果を合成点として本番の応答型を共有します。A案の他のパーツは将来の実装位置を示す固定サンプルであり、APIの応答には含めません。
