...
UC-1とUC-2の承認済み設計を記録します。実装済みの基盤は [React構成](architecture-react.md)、テンプレート採用範囲は [文書方針](document-policy.md#adoption) を参照します。

## 全体設計の合意

リポジトリ所有者が第1〜4節の全体構造と、UCP-1・UCP-2を承認しています。テーブル設計は第5節で別途合意します。最新の確認後保存コミットは `5fbbc6f`（UC-1-X1の通信断時再接続をUCP-1へ追加）です。
...
適用系列: [UC-2-M・UC-2-X1・UC-2-X2・UC-2-X3・UC-2-X4・UC-2-X5](usecases/UC-2.md)。現在の接続設定に登録されているHubだけを表示対象とし、表示処理ではHub数を固定しません。
...
UC-1-Mで `hubs` と `hub_states` を新設し、スキーマ版を0から1へ移行しました。実装済みのDBスキーマ版は1で、以降の系列による変更はありません。Hubごとに通知を反映済みの最新状態を1行で保持し、受信履歴は蓄積しません。
...
合意記録: UC-1-M / 2026-09-20 / 判断者: リポジトリ所有者 / 確認後保存コミット: `8d6669e` / 決定: Hubを独立した実体として `hubs` と `hub_states` に分け、表示名と0または1の関係、起動時登録を採用する。最新状態はJSONで保持し、スキーマ版0から1へ移行する。
