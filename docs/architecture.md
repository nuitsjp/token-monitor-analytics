...
したがって、受信イベント数をそのまま利用回数として扱うことはできない。本システムでは「現在値の観測（SSE）」と「日次・月次実績の収集（GET）」を完全に分離して独立に受信・処理する（詳細は第 6 節参照）。
...
また、日次履歴（`daily`）は送信元の現地暦日ごとの集計値であり、タイムゾーン情報や利用時刻の明細を持たない。Hub の履歴集約も同じ日付の行を合算する処理であり、共通タイムゾーンへの再集計ではない。そのため、受信側でのタイムゾーン変換や按分は行わず、端末の現地日付を維持して扱う（設計原則は第 3.6 節参照）。
...
| Hub ごとの収集設定（有効・停止） | 再起動後も利用者が指定した設定を維持 | Analytics 上の設定のため再取得不可 |
...
| U6 | 受信処理とタイムゾーン | SSE と GET を独立受信し単一キューで直列保存。日次履歴は現地日付を維持。Hub ごとの収集設定を SQLite に保存し、再起動後も引き継ぐ（第 6.3 節）。 | タイムゾーン欠落時の当日判定、収集設定を含む SQLite スキーマ、トランザクション境界。 |
...
初回接続・再接続時は `GET /api/stats/stream` の SSE 接続を直ちに開始し、現在値の観測・表示を優先する。履歴収集（`GET /api/devices`）は以下のタイミングで独立して実行する。

| タイミング・状況 | 動作仕様 |
| --- | --- |
| 初回接続時 | Hub が保持する全履歴を取得・保存する。 |
| 稼働中の定期実行 | 毎日午前 1 時（Analytics 稼働マシンの現地時刻）に、Hub ごとに保持履歴を取得する。 |
...
| 再起動時の収集設定の復元 | 収集が有効な Hub と停止した Hub を混在させて再起動し、有効な Hub は自動接続、停止した Hub は明示的な再開まで全収集通信を停止したままであること。接続状態は起動後の通信で判断し、有効な Hub に接続できなければ「未接続」と表示して自動再接続を続けること。 |
| 前日データ不在 | データ不在が明示され、ゼロ埋めや過剰な再試行が発生しないこと。 |
| 過去実績の遅延反映 | 遅れて報告された実績が次回以降の GET で反映され、過去データや推定結果が破壊されないこと。 |
...
| 収集停止と再開 | 停止操作により対象 Hub の全通信が停止し、再開操作で正常に復帰すること。他 Hub の収集や履歴閲覧は維持されること。 |
| 処理中の収集停止 | 停止要求前に受信完了したデータは正常に保存され、未完了の通信は取り込まれないこと。処理完了後に「収集停止」と表示されること。 |

### 6.3 Analytics 上での収集停止と再開

利用者が Analytics 上で特定の Hub からの収集を停止・再開する際の動作仕様を定める（上流 Hub 自体の稼働状態とは独立して管理する）。

| 区分 | 状態と動作 |
| --- | --- |
| 未接続 | 収集停止操作を行っていない状態で Hub に接続できない場合。原因（Hub 障害か意図的な停止か）を問わず自動再接続を継続する。 |
| 収集停止 | 利用者が停止操作を行った場合。再開操作まで該当 Hub の SSE 接続・自動再接続・定期 GET・GET 再試行を停止する。 |

収集停止中も、保存済みデータの閲覧や他 Hub の収集は通常どおり継続する。

#### 停止操作時のデータ処理境界
停止時のデータ取り込みは、**Analytics が停止要求を受け付けた時点** を境界とする（1 回の GET 応答または 1 回の SSE データ通知単位で判定）。

- **受信完了済みのデータ**: 共通キューで待機中・処理中のものを含め、通常どおり処理・保存する。
- **受信途中の通信**: 完了を待たずに中止し、遅れて届いた応答は破棄する。
- **処理完了後の表示**: 受信済みデータの処理終了後、該当 Hub を「収集停止」と表示する。保存に失敗した場合は書き込みを停止し、「収集停止」と「保存失敗」を併記する。

#### 収集設定の保存と再起動時の復元

Hub ごとに利用者が指定した収集設定（有効・停止）を SQLite に保存し、Analytics の再起動後も引き継ぐ。起動時は Hub との通信を開始する前に設定を読み込み、再起動のたびに停止・再開を操作し直す必要をなくす。

| 保存された収集設定 | 起動後の動作 |
| --- | --- |
| 有効 | 自動で SSE 接続を開始し、接続できない場合は「未接続」と表示して自動再接続を続ける。履歴 GET は第 6.1 節の取得・再試行方針に従う。 |
| 停止 | 「収集停止」を維持し、利用者の明示的な再開操作まで SSE 接続・自動再接続・定期 GET・GET 再試行を停止したままにする。 |

再起動後の収集動作は、この収集設定に従う。実際の接続状態は起動後の通信結果から判断し、再起動前の接続状態を現在の状態として表示しない。
