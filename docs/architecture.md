...
- UI確認用モックは不要です。検証時の置き換え境界は外部Hubとし、制御可能なSSEサーバーから本番の受信・保存・配信処理を通し、別のDB接続の保存値と閲覧側の受信内容を照合します。保存後通知は段階4の手動確認済みで、改訂後のE2E・実Hub検証は未実施です。詳細は [検証結果](project.md#verification) を参照します。
