...
上流リポジトリ [Token Monitor](https://github.com/Javis603/token-monitor) を `upstream/token-monitor` に Git サブモジュールとして登録し、ソースコードおよび実環境での応答を調査した（サブモジュールは仕様調査用であり実行時依存ではない）。
...
| `GET /api/subscriptions` | Hub 内で共有される手入力の契約・支払情報。 | 実績と契約の確実な紐付け証拠とはならないため、初期版では不採用。 |

- **認証方式**: 保護経路は共有シークレットを `Authorization: Bearer <secret>` または `X-Token-Monitor-Secret: <secret>` ヘッダーで受け取る（URL に含めずヘッダーで送信）。本システムの Web 画面の Basic 認証とは独立して管理する。
- **未設定・公開経路の挙動**:
...
したがって、受信イベント数をそのまま利用回数として扱うことはできない。本システムでは「現在値の観測（SSE）」と「日次・月次実績の収集（GET）」を完全に分離して独立に受信・処理する（第 7 節）。
...
※ 同一 Hub 内で過去データの `deviceId` を新しい `deviceId` に書き換えて手動関連付けを行う機能は将来要件とし、初期版には実装しない。

### 3.2 再受信と二重計上防止

- **重複排除**: 同一内容の再受信によって新たな増分や推定値を計上せず、同一日次・月次実績の再取得時も加算を行わず上書き更新する。受信時刻や通信経路の違いを新規実績の根拠としない。
- **端末間複製ログの非サポート**: 同一の利用ログファイルを複数端末にコピー・共有して Hub へ多重報告する構成はサポート対象外とする（同一アカウントを複数端末で独立して利用する構成はサポート対象）。
...
| U6 | 受信処理とタイムゾーン | SSE と GET を独立受信し単一キューで直列保存。日次履歴は現地日付を維持。接続失敗時は自動再接続し、Analytics 上での明示的な収集停止時は再開操作まで収集を止める（第 7.3 節）。 | タイムゾーン欠落時の当日判定、SQLite スキーマ、トランザクション境界、収集状態の保存・復元と停止操作時の受信・保存の競合。 |

## 6. 調査結果サマリー

上流リポジトリのコード調査および Private Hub での実機検証により、更新タイミング、正規化による属性欠落、履歴保持の制約等を把握した。これらに基づき、第 3 節の識別・推定原則および第 5 節の基本方針を確定した。
...
#### 単一キューによる直列化の理由
並行受信時に同一の比較基準を同時に読み書きすると、不整合や重複計算の原因となる。これを防ぐため、照合から保存までの一連の処理を直列に実行する。通信の待機時間はキューの外側で並行処理するため、システム全体の遅延要因とはならない。保存失敗時は更新通知を抑制し、安全に書き込みを停止する。

### 7.1 接続制御と履歴の定期取得

初回接続・再接続時は `GET /api/stats/stream` の SSE 接続を直ちに開始し、現在値の観測・表示を優先する。履歴収集（`GET /api/devices`）は以下のタイミングで独立して実行する。Analytics 上で収集を停止した Hub は、再開操作まで SSE 接続・自動再接続および以下の GET 実行の対象外とする（第 7.3 節）。
...
| 当日日次・当月月次 | 当日日次は保存除外され、当月月次は「途中集計」として扱われること。SSE 現在値が日次へ転記されないこと。 |
...
| 上流 Hub の停止による接続失敗 | Hub を障害で停止させた場合も、上流の利用者が停止させた場合も、Analytics 上で収集を停止していなければ未接続と表示し、自動再接続を継続すること。 |
| Analytics 上での収集停止・再開 | 停止操作により該当 Hub の SSE 接続・自動再接続・定期 GET・GET 再試行が停止すること。保存済みデータの閲覧と他 Hub の収集は維持し、明示的な再開操作で収集を再開すること。 |

### 7.3 Analytics 上での収集停止と再開

「Hub ごとの収集停止」は、利用者が Analytics 上でその Hub からのデータ取得を停止する操作を指す。上流 Hub 自体を停止・起動する機能ではない。Analytics は上流 Hub の停止理由を判別できないため、接続状態と、Analytics 自身が受け付けた収集停止・再開の操作を区別して扱う。

| 区分 | 判定できる事実と動作 |
| --- | --- |
| 1. Hub への接続失敗・通信切断 | 収集停止操作がなく Hub に接続できない場合は「未接続」と表示し、原因を問わず自動再接続を継続する。Hub の障害停止と、上流の利用者による意図的な停止は区別しない。履歴 GET の通信失敗は第 7.1 節の再試行方針に従う。 |
| 2. Analytics 上での収集停止 | Analytics 自身が受け付けた利用者の操作で判定する。再開操作まで、その Hub の SSE 接続・自動再接続・定期 GET・GET 再試行を停止し、「収集停止」として未接続と区別する。 |

収集停止中も保存済みデータは閲覧でき、他 Hub の収集は継続する。再開操作は Analytics 側の収集を有効に戻す操作である。再開後に Hub へ接続できない場合は区分 1 に従う。

収集状態の保存形式・再起動時の復元手順、停止操作と受信・保存が重なった場合の未保存データの扱いは後続設計で具体化する。
