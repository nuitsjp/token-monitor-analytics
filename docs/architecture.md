...
- **残る検証**: 実Hubの通信切断・自動再接続、別端末からのJSON API直接取得・SSE再接続、実ディスク・権限障害、長時間稼働を含む従来方式の実機検証を [PLAN.md](../PLAN.md) で追跡します。履歴補完・履歴画面の実装と検証は [U6](../PLAN.md#u6) で完了しました。Web管理、手動更新は未実装です。

- **U6修正の検証（schema 10、2026-09-14）**: 全202テストが成功し、入力不正による保存停止、旧schemaの移行、通信再試行、停止・再開の競合、同日・同月内のページング、履歴取得情報の保存・二期間比較を検証しました。実DB複製38,461観測・Mock複製780観測で旧11表の件数と共通推定checkpointの一致、integrity_check・foreign_key_check正常を確認しました。Private・Workから日次662行・月次128行をメモリDBへ保存し、全ページの重複なし・日次の当日分除外を確認しました。Windowsの一時ヘッドレスEdgeで221件・3ページの絞り込み・比較・追加表示、読み出し失敗時の保持、取得状態を検証し、390px・1440pxでページ全体の横はみ出しがないことを確認しました。証跡は [PLAN.md](../PLAN.md#u6) に記録しています。
