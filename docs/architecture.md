...
合成点は `frontend/src/features/usage-overview.ts` の取得・購読境界です。本番の `UsageOverview` を共有し、初回取得はtRPC、以後は同一originの `/api/usage/stream` を使います。仕様合意用の固定データとモック切替は削除済みです。UC-2-X3では期間別 `clients`、UC-2-X4では期間別 `models` をAPI応答と通知へ含め、画面で全Hubを合算します。利用枠は保存済み Hub 集約 `limits` から表示します。トレンド・アクティビティは固定サンプルです。DB変更はありません。
