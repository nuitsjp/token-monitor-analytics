...
合成点は `frontend/src/features/usage-overview.ts` の取得・購読境界です。本番の `UsageOverview` を共有し、Vite の `mock` モードだけが固定データに差し替えます。本番経路では tRPC と `/api/usage/stream` を使います。トレンド・ツール・モデル・アクティビティは固定サンプルです。DB変更はありません。
