...
| 役割         | 責務                                                                                              | 実装パス    |
| ------------ | ------------------------------------------------------------------------------------------------- | ----------- |
| 利用状況読取 | 登録Hubと最新状態を読み、Hub・デバイス・モデル・利用枠の表示に必要な値だけを返す                    | backend/http/router.ts、backend/db/hub-state.ts |
...
合成点は `frontend/src/features/usage-overview.ts` の取得・購読境界です。本番の `UsageOverview` を共有し、初回取得はtRPC、以後は同一originの `/api/usage/stream` を使います。仕様合意用の固定データとモック切替は削除済みです。トレンド・ツール・アクティビティは固定サンプルです。DB変更はありません。
