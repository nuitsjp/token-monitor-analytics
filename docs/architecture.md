...
UC-1の合意済み設計を記録します。Hub受信・保存を実装済みです。完成系の承認と段階6の検証は未実施です。実装済みの基盤は [React構成](architecture-react.md)、テンプレート採用範囲は [文書方針](document-policy.md#adoption) を参照します。
...
| 役割 | 責務 | 実装パス |
| --- | --- | --- |
| 起動・終了管理 | 設定・DBを準備し、Hubを登録してから受信開始。終了時は受信を止めてからDBを閉じる | backend/app.ts、backend/main.ts、backend/config.ts |
