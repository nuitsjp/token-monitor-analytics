...
U2・U3・U5 の初期方針と U4 の期間判定方針は合意済みである。手順 1 には U2 の具体的な識別設計と U4 の対象サービス・プラン・枠の識別詳細が残る。U3 の測定時刻のずれを許容する方針について、連続観測による許容秒数の決定を初期設計の完了条件にはしない。
...
今回の調査で上流のデータ形式と補完の限界を特定し、利用者との検討により U2・U3・U5 の初期方針と U4 の期間判定方針を確定した。手動での端末関連付けを将来要件として記録した。手順 1 全体は、残る対象・識別の設計を終えていないため未完了である。
