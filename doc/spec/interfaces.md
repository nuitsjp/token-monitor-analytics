...
保護経路には共有シークレットを `Authorization: Bearer <secret>` または `X-Token-Monitor-Secret: <secret>` ヘッダーで送信します（URL クエリへは含めない）。シークレット未設定時の上流挙動は、Node Hub ではループバック接続のみ認証不要、Worker Hub では保護経路を HTTP 503 で拒否する構成でした。公開統計 `/api/public/stats` は識別情報が不足するため利用しません（Private Hub の単発取得では HTTP 404）。
...
| `kind` / `limitId` / `additional` | 枠識別の補助情報（単独項目での一意性は保証されない。同一 `kind` にモデル別枠、同一 `limitId` に期間別枠が併存しうる） |
| `usedPercent` | 0〜100 に正規化された消費率（未取得時は `null` であり、ゼロや推測値で補完しない） |
...
調査した Claude ではアカウント ID・組織 ID・メールアドレスから `accountKey` を生成しており、同一アカウント ID 内での組織切替がキーへ反映されない場合があります。また Codex ではメールアドレスとワークスペースのアカウント ID からキーを生成します。これらはアカウントキーを契約 ID として代用できない根拠となります。
