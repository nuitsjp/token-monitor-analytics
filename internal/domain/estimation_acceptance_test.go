...
func TestACPhase1EstimationFixtures(t *testing.T) {
	...
	t.Run("AC-P1-14 two points estimate 100", func(t *testing.T) {
		...
		assertEstimation(t, result, EstimationEstimated, 100)
	})
	...
	t.Run("AC-P1-16 least squares 150 is estimated", func(t *testing.T) {
		result, err := EstimateFromPoints(EstimationInput{Points: t032SingleThreePoints([]float64{0, 0.1, 0.2}, []float64{0, 10, 30})})
		if err != nil {
			t.Fatal(err)
		}
		assertEstimation(t, result, EstimationEstimated, 150)
	})
	...
	t.Run("AC-P1-19 zero signal and positive unique solution", func(t *testing.T) {
		...
		positiveUnique, err := EstimateFromDifferences(mat.NewDense(2, 1, []float64{0.1, 0.1}), []float64{9, 11})
		if err != nil {
			t.Fatal(err)
		}
		if positiveUnique.Status != EstimationEstimated {
			t.Fatalf("positive unique result = %#v", positiveUnique)
		}

		nonPositive, err := EstimateFromDifferences(mat.NewDense(2, 2, []float64{1, 2, 2, 1}), []float64{1, 10})
		if err != nil {
			t.Fatal(err)
		}
		if nonPositive.Status != EstimationModelMismatch || len(nonPositive.Reasons) == 0 || nonPositive.Reasons[0] != "non_positive_solution" {
			t.Fatalf("non-positive solution result = %#v", nonPositive)
		}
	})
	...
	t.Run("P1-EST-20 exactly identified positive solution is estimated", func(t *testing.T) {
		result, err := EstimateFromPoints(EstimationInput{Points: t032SinglePoints([]float64{0.1}, []float64{0.3}, 10, 30)})
		if err != nil {
			t.Fatal(err)
		}
		if result.Status != EstimationEstimated || len(result.Limits) != 1 {
			t.Fatalf("estimated result = %#v", result)
		}
		closeEnough(t, result.Limits[0], 100)
	})
	t.Run("P1-EST-21 overidentified positive solution is estimated", func(t *testing.T) {
		result, err := EstimateFromPoints(EstimationInput{Points: t032SingleThreePoints([]float64{0.1, 0.3, 0.4}, []float64{10, 30, 40})})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("P1-EST-22 non-positive solution is model mismatch", func(t *testing.T) {
		result, err := EstimateFromDifferences(mat.NewDense(2, 2, []float64{1, 2, 2, 1}), []float64{1, 10})
		if err != nil {
			t.Fatal(err)
		}
		if result.Status != EstimationModelMismatch || len(result.Reasons) == 0 || result.Reasons[0] != "non_positive_solution" {
			t.Fatalf("model mismatch result = %#v", result)
		}
	})
}

func assertEstimation(t *testing.T, result EstimationResult, status EstimationStatus, limit float64) {
	t.Helper()
