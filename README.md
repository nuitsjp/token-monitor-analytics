...
目標製品は Windows 向けのローカルファーストな Wails v3 デスクトップアプリです。現在このリポジトリで動作するコードは `poc/` に退避した検証用実装であり、目標要件を満たす Phase 1 実装ではありません。
...
- 両方を正式なサービス、論理アカウント、利用枠定義へ別々に関連付け、同じ計算区間の観測だけを推定に使用します。
...
必要最小限の独立した差分から得た値は暫定推定、余分な差分も総絶対誤差率 10% 以下で説明できた値は検証済み推定です。観測不足、階数不足、ゼロ信号、モデル不適合では、均等配分、補間、外挿、外れ値の自動除外を行いません。
...
| 段階 | 範囲 | 現在の状態 |
| --- | --- | --- |
| Phase 1 | 複数 Hub の収集、ローカル観測正本、識別・関連付け、差分推定、履歴、明示パージ、バックアップと復元 | 要件定義済み、未実装 |
| Phase 2 | 標準価格との比較、利用実績分析、可視化、CSV・JSON エクスポート | Phase 1 完了後に実装 |
| クラウド複製 | ローカル観測正本からの一方向複製 | 初期スコープ外 |

Phase 1 と Phase 2 には別々の完了ゲートがあります。試作機能が存在するだけでは、どちらの完了ともみなしません。
